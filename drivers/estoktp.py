...
if MECH_TYPE == 'CHEMKIN':
    ...
    if os.path.exists(os.path.join(MECH_PATH, 'class.csv')):
        CLA_STR = open(os.path.join(MECH_PATH, 'class.csv')).read()
        CLA_STR = CLA_STR.replace(' ', '')
    else:
        ...
        if pes_idx in PARAMS.PESNUMS:
            ...
            for cidx, cvals in enumerate(connchnls.values()):
                ...
                if run_pes:
                    ...
                    # Build list for the energy transfer params for ktpdriver
                    etrans_lst = [
                        PARAMS.EXP_FACTOR,
                        PARAMS.EXP_POWER,
                        PARAMS.EXP_CUTOFF,
